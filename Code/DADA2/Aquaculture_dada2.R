##########################################
### V4V5 processing of aquaculture samples
### from Bartelme et al., 2017
### Lou LaMartina, started Jan 29, 2020
##########################################
...
setwd("~/Desktop/Aeromonas/Aquaculture")
...
allF <- sub("R1.fastq.gz", "", sub("R1_001.fastq.gz", "", list.files(pathF)))
allR <- sub("R2.fastq.gz", "", sub("R2_001.fastq.gz", "", list.files(pathR)))
    
fastqFs <- c(sort(list.files(pathF, pattern = "R1_001.fastq.gz", full.names = TRUE)),
             sort(list.files(pathF, pattern = "R1.fastq.gz", full.names = TRUE)))
fastqRs <- c(sort(list.files(pathR, pattern = "R2_001.fastq.gz", full.names = TRUE)),
             sort(list.files(pathR, pattern = "R2.fastq.gz", full.names = TRUE)))
...
# did not truncate / too much was removed
...
write.csv(data.frame(Sample_name = rownames(Sequence_table_nochim), Sequence_table_nochim), 
          "Aquaculture_counts.csv", row.names = FALSE)
write.csv(data.frame(FASTA = rownames(Sequence_taxa_table), Sequence_taxa_table), 
          "Aquaculture_taxa.csv", row.names = FALSE, na = "")
write.csv(data.frame(Sample_name = rownames(Sequence_table_relabun), Sequence_table_relabun), 
          "Aquaculture_relabun.csv", row.names = FALSE)




# # # # # # # # # # # # # # # # # # # # # # # # #
save.image("Aquaculture_dada2_environment.RData")
# # # # # # # # # # # # # # # # # # # # # # # # #
